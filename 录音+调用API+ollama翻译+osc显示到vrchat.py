...
def send_text_to_vrchat(original_text, translated_text):
    ...
    if translated_text: # 确保有译文才发送
        ...
        # VRChat 聊天框对消息长度有限制，过长的消息可能无法完整显示或被截断
        # 通常限制在144字节左右 (UTF-8编码下，一个中文字符通常占3字节)
        # 这里简单截断，实际应用中可能需要更复杂的处理
        max_bytes = 140 
        message_bytes = message_to_send.encode('utf-8')
        if len(message_bytes) > max_bytes:
            # 尝试按字符截断，并确保utf-8编码后不超过max_bytes
            # 这是一个简化的截断，可能不是最优的
        ...
        try:
            osc_client.send_message("/chatbox/input", [message_to_send, True, False]) # [message, send_now=True, notification_sound=False]
...
def process_transcription_and_translate(transcribed_text):
    ...
    translated_text_content = None # 初始化

    if selected_ollama_model and target_language_name:
        ollama_target_lang_prompt_name = target_language_name.split(" (")[0]
        translated_text_content = translate_text_with_ollama(transcribed_text, selected_ollama_model, ollama_target_lang_prompt_name)
        
        if translated_text_content:
            print(f"\n译文 ({target_language_name}):")
            print(translated_text_content)
            print("----------------------------------------")
        else:
            print("\n翻译失败或未返回有效译文。")
            print("----------------------------------------")
